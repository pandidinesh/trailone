...
// Simulate login user
if(!isset($_SESSION['email'])) $_SESSION['email'] = 'dinesh@example.com';
...
if(isset($_POST['action'])){
    ...
    if($_POST['action']=='get_messages'){
        ...
        $res = mysqli_query($conn,"SELECT * FROM messages 
            WHERE (sender_email='$my_email' AND receiver_email='$chat_with')
               OR (sender_email='$chat_with' AND receiver_email='$my_email')
...
function loadMembers(){
    $.post('chat.php',{action:'get_members'},function(data){
        let members = JSON.parse(data);
        let html='';
        members.forEach(m=>{
            html += `<div class="member" data-email="${m.email}">${m.name}</div>`;
        });
        $('#members').html(html);
    });
}
...
function loadMessages(){
    ...
    $.post('chat.php',{action:'get_messages',chat_with:selectedMemberEmail},function(data){
...
$('#sendBtn').click(function(){
    ...
    $.post('chat.php',{action:'send_message',to:selectedMemberEmail,msg:msg},function(res){
        $('#msgInput').val('');
        loadMessages();
    });
});

// Auto refresh members & messages
loadMembers();
