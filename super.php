...
// Simulate login user (normal login system la $_SESSION['email'] set pannuvanga)
if(!isset($_SESSION['email'])) $_SESSION['email']='dinesh@example.com';
if(!isset($_SESSION['name'])) $_SESSION['name']='Dinesh';
...
function loadMembers(){
    $.post('dashboard.php',{action:'get_members'},function(data){
        ...
        let html='';
        members.forEach(m=>{ html += `<div class="member" data-email="${m.email}">${m.name}</div>`; });
        $('#members').html(html);
    });
}
